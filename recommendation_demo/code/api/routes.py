...
from Tag_NewContent import getHtml,get_data
from Tag_Clustering_Hourly_Health import return_post
from Tag_Clustering_Hourly_Health_dev import return_post as return_post_dev
...
@health.route('/content_dev')
def content_dev():
    minute = datetime.now().minute
    table = cache.get('health_content_dev')
    day = request.args.get('day', 1, type = int)
    if not table or (minute in [0,15,30,45]):
        content = get_data('health_content_dev', 1080)
        table = content.to_json(force_ascii=False)
        cache.set('health_content_dev',table,timeout=900)
...
def content():
    ...
    if not table or (minute in [0,15,30,45]):
        ...
        table = content.to_json(force_ascii=False)
        cache.set('health_content',table,timeout=900)
        filt_tmp = content[content['date']>=date.today() - timedelta(days=day)]
        filt_tmp = filt_tmp.drop(['date'], axis=1)
        filt = filt_tmp.to_json(force_ascii=False)
        return filt
...
@health.route('/content_update_dev')
def content_update_dev():
    minute = datetime.now().minute
    table = cache.get('health_content_update_dev')
    if not table or (minute in [0,15,30,45]):
        content = get_data('health_content_update_dev', 1080)
        table = content.to_json(force_ascii=False)
#        cache.set('health_content_update',table,timeout=3600)
        return table
    else:
        return table


@health.route('/content_update')
def content_update():
    minute = datetime.now().minute
    table = cache.get('health_content_update')
    if not table or (minute in [0,15,30,45]):
        content = get_data('health_content_update', 1080)
        table = content.to_json(force_ascii=False)
#        cache.set('health_content_update',table,timeout=3600)
        return table
    else:
...
@health.route('/tag_score_table', methods=['GET'])## 標籤分數表API
#    @cache.cached(timeout=5)
def tvbs_tag_analysis():
    hour = datetime.now().hour
    minute = datetime.now().minute
#    args = request.args
...
#google search console 資料 cache
...
def gsc_tag():
    ...
    if not gsc_table or \
        ...
        ((hour>20 and minute > 0) and (hour>20 and minute <= 1)) or ((hour>22 and minute > 0) and (hour>22 and minute <= 1)):
        db_config.gcp_db()
        insert = """SELECT article_id,search_content FROM MartServer.GSC_Content Where domain = "Health";"""
        conn = mysql.connect()
        cur = conn.cursor(pymysql.cursors.DictCursor)
        cur.execute(insert)
        rows = cur.fetchall()
        tag_gsc = pd.DataFrame(rows)
        tag_gsc.set_index('article_id',inplace=True)
        cur.close()
        conn.close()
...
@health.route('/post_tag_recommend',methods=['POST'])##health 推薦文章API
...
#        now_time = now.time()
    table = cache.get('gsc')
    if not table:
        content_all = pd.read_json(getHtml('http://34.80.91.60:5050/health/content?day=1080'))
        tmp = get_data('gsc', '\',\''.join(list(content_all['article_id'].astype(str))), 'Health')
        table = tmp.to_json(force_ascii=False)
#            cache.set('news_content_tmp',content,timeout=86100)
...
@health.route('/recommend_list_dev', methods=['GET'])
def result_dev():
    minute = datetime.now().minute
    table = cache.get('recommend_dev')
#        article_id = request.args.get('article_id', 1, type = int)
    if not table or (minute > 0 and minute <= 5) or (minute > 15 and minute <= 20) or (minute >= 30 and minute <= 35) or (minute >= 45 and minute <= 50):
        tmp = get_data('recommend_list_dev',domain = 'Health')
        table = tmp.to_json(force_ascii=False)
#            cache.set('news_content_tmp',content,timeout=86100)
        cache.set('recommend_dev',table,timeout=86100)
...
@health.route('/post_recommend_dev',methods=['POST'])##News 推薦文章API
def return_recommend_dev():
    temp_json = request.get_json(force=True)
    recommend_list = return_post_dev(temp_json['text'])
